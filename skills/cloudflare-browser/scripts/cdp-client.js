...
function createClient(options = {}) {
  ...
  const workerUrl = (options.workerUrl || process.env.WORKER_URL).replace(
    /^https?:\/\//,
    ""
  );
    ...
    ws.on("open", async () => {
      try {
        ...
        await Promise.race([
          targetReady,
          new Promise((_, rej) =>
            setTimeout(() => rej(new Error("No target created")), 10000)
          )
        ]);
...
if (require.main === module) {
  console.log(
    'CDP Client Library - import with: const { createClient } = require("./cdp-client")'
  );
}
