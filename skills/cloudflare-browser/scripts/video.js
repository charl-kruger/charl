...
const output =
  args.find((a, i) => i > 0 && !a.startsWith("--")) || "output.mp4";
const fps = args.includes("--fps")
  ? parseInt(args[args.indexOf("--fps") + 1])
  : 10;
...
async function main() {
  ...
  await Promise.race([
    ...
    new Promise((_, reject) =>
      setTimeout(() => reject(new Error("No target created")), 10000)
    )
  ]);
  ...
  try {
    ...
    console.log(
      `✓ Video saved to ${outputPath} (${(stats.size / 1024).toFixed(1)} KB)`
    );
  } catch (err) {
