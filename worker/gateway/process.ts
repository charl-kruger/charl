...
export async function ensureMoltbotGateway(
  ...
  if (existingProcess) {
    console.log(
      "Found existing Moltbot process:",
      existingProcess.id,
      "status:",
    ...
    try {
      console.log(
        "Waiting for Moltbot gateway on port",
        MOLTBOT_PORT,
        "timeout:",
        STARTUP_TIMEOUT_MS
      );
  ...
  const backupDir = `/data/moltbot/${agentId}`;
  const envVars = {
    ...buildEnvVars(env),
    ...agentSecrets,
    MOLTBOT_BACKUP_DIR: backupDir
  };
  ...
  try {
    console.log(
      "[Gateway] Waiting for Moltbot gateway to be ready on port",
      MOLTBOT_PORT
    );
    ...
    try {
      ...
      throw new Error(
        `Moltbot gateway failed to start. Stderr: ${logs.stderr || "(empty)"}`
      );
    } catch (logErr) {
